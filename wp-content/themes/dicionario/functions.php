...
function wpdl_show_paginated_hands($local, $MAOS_POR_PAGINA, $my_term=0) {
  ...
  if($local=="home"){
    if ($paginas > 0) {
      ...
      echo '<img src="'.esc_url(get_template_directory_uri()).'/img/left-home.png" class="glyphicon glyphicon-chevron-left" style="width:30px" />';
      echo '</a>';
      echo '<a class="right carousel-control" href="#carousel-example-generic" role="button" data-slide="next">';
      echo '<img src="'.esc_url(get_template_directory_uri()).'/img/right-home.png" class="glyphicon glyphicon-chevron-right" style="width:30px" />';
      echo '</a>';
    }
  }else if($local=="cadastros"){
    if ($paginas > 0) {
      echo '<a class="left carousel-control" href="#carousel-example-generic" role="button" data-slide="prev">';
...
function search_js() {
  ...
  wp_enqueue_script('search-js', get_stylesheet_directory_uri().'/js/search.js', array('jquery', 'autocomplete'));
...
function ajax_listings() {
  ...
  $sql = "select post_name, post_title
    from $wpdb->posts 
  ...
  foreach( $results as $r ){
    $posts['titles'][] = $r->post_title;
    $posts['slugs'][] = $r->post_name; 
  }
    
  echo json_encode($posts); //encode into JSON format and output
