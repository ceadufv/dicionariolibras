 ...
 * Mimics Walker_Category_Checklist excerpt for the checkbox input
 *
 * @see Walker
 ...
 * @since 2.5.1
 */
class Walker_Category_checkbox extends Walker {
	...
	function start_el( &$output, $category, $depth = 0, $args = array(), $id = 0 ) {
		...
		$name = 'checkbox_tax_input['.$taxonomy.']';
		...
		// switching checkbox tags to "hierarchical" so we'll always use ID
		$value = $category->term_id;

		$class = in_array( $category->term_id, $popular_cats ) ? ' class="popular-category"' : '';

		$output .= sprintf( "\n" . '<li id="%1$s-%2$s" %3$s><label class="selectit"><input id="%4$s" type="checkbox" name="%5$s" value="%6$s" %7$s %8$s/> %9$s</label>' ,
			$taxonomy, //1
			$value, //2
			$class, //3
