...
if( ! class_exists( 'WordPress_checkbox_Taxonomy' ) ) :

class WordPress_checkbox_Taxonomy {
	...
	*/
	public function __construct( $taxonomy ){
		...
		// change checkboxes to checkboxs & trigger get_terms() filter
		...
		// hack global taxonomy to switch all checkbox taxonomies to hierarchical on edit screen
	...
	public function add_meta_box() {
		if( ! is_wp_error( $this->tax_obj ) && isset($this->tax_obj->object_type ) ) foreach ( $this->tax_obj->object_type as $post_type ):
			...
			$id = ! is_taxonomy_hierarchical( $this->taxonomy ) ? 'checkbox-tagsdiv-' . $this->taxonomy : 'checkbox-' . $this->taxonomy . 'div' ;
	...
	public function metabox( $post, $box ) {
		...
		?>
		<div id="taxonomy-<?php echo $taxonomy; ?>" class="checkbox-for-taxonomies categorydiv form-no-clear">
			...
			<?php wp_nonce_field( 'checkbox_nonce-' . $taxonomy, '_checkbox_nonce-' . $taxonomy ); ?>
				...
				<ul id="<?php echo $taxonomy; ?>checklist-pop" class="categorychecklist form-no-clear" >
					<?php $popular = get_terms( $taxonomy, array( 'orderby' => 'count', 'order' => 'DESC', 'number' => 10, 'hierarchical' => false ) );
						...
						<?php foreach( $popular as $term ){
							...
							echo "<input type='checkbox' id='in-{$id}'" . checked( $single_term_id, $term->term_id, false ) . " value='{$value}' {$disabled} />&nbsp;{$term->name}<br />";
					...
					<h4>
						<a id="<?php echo $taxonomy; ?>-add-toggle" href="#<?php echo $taxonomy; ?>-add" class="hide-if-no-js">
							<?php
								...
								printf( __( '+ %s' , 'checkbox-for-taxonomies' ), $this->tax_obj->labels->add_new_item );
							?>
						</a>
					</h4>
	...
	public function filter_terms_checklist_args( $args ) {
		...
		if( isset( $args['taxonomy']) && $this->taxonomy == $args['taxonomy'] ) {
			...
			// add a filter to get_terms() but only for checkbox lists
			$this->switch_terms_filter(1);
			add_filter( 'get_terms', array( $this, 'get_terms' ), 10, 3 );

			$args['walker'] = new Walker_Category_checkbox;
	...
	private function switch_terms_filter( $_set = NULL ) {
		...
		return apply_filters( 'checkbox-for-taxonomies-no-term-' . $this->taxonomy, $this->set );
	}
	...
	function get_terms( $terms, $taxonomies, $args ){
		...
		// only filter terms for checkbox taxes (except category) and only in the checkbox - need to check $args b/c get_terms() is called multiple times in wp_terms_checklist()
		if( in_array( $this->taxonomy, ( array ) $taxonomies ) && ! in_array( 'category', $taxonomies ) 
			...
			$no_term = sprintf( __( 'No %s', 'checkbox-for-taxonomies' ), $this->tax_obj->labels->singular_name );
	...
	public function add_non_hierarchical_term(){
		...
		foreach ( $names as $cat_name ) {
			...
			$data = sprintf( '<li id="%1$s-%2$s"><label class="selectit"><input id="in-%1$s-%2$s" type="checkbox" name="checkbox_tax_input[%1$s][]" value="%2$s" checked="checked"> %3$s</label></li>', $taxonomy->name, $cat_id, $cat_name );
	...
	function save_single_term( $post_id ) {
		...
		// verify nonce
		if ( isset( $_POST["_checkbox_nonce-{$this->taxonomy}"]) && ! wp_verify_nonce( $_REQUEST["_checkbox_nonce-{$this->taxonomy}"], "checkbox_nonce-{$this->taxonomy}" ) ) 
			return $post_id;

		// OK, we must be authenticated by now: we need to find and save the data
		if ( isset( $_REQUEST["checkbox_tax_input"]["{$this->taxonomy}"] ) ){

			$terms = (array) $_REQUEST["checkbox_tax_input"]["{$this->taxonomy}"]; 
	...
	/**
	 * Use this action to switch all checkbox taxonomies to hierarchical on edit.php
	...
	public function quick_edit_nonce() {
		if ( $this->printNonce ) {
			...
			wp_nonce_field( 'checkbox_nonce-' . $this->taxonomy, '_checkbox_nonce-' . $this->taxonomy );
		}
